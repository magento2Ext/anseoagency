...
<section class="seocontent_sec">
        <div class="wrapper">
                    ...
                    <div class="serviceimgbx fleft">
                        <?php
            $image = get_field('ppc_service_explained_group_service_explained_left_image');
            if ($image) {
            echo '<img src="' . esc_url($image['url']) . '" alt="' . esc_attr($image['alt']) . '">';
            }
            ?>
                    ...
                    <div class="servicecontentbx fright ptag">
                        <h2><?php the_field('ppc_service_explained_group_service_explained_right_heading');?></h2>
                      <?php the_field('ppc_service_explained_group_explained_service_right_content');?>  
                    </div>
                </div>
                <div class="servicecontent_div maketopspacing">
                    <div class="servicecontentbx fleft ptag">
                        <h2><?php the_field('ppc_service_explained_group_service_explained_left_heading');?></h2>
                        <?php the_field('ppc_service_explained_group_service_explained_left_content');?>
                    </div>
                    <div class="serviceimgbx fright">
                        <?php
            $image = get_field('ppc_service_explained_group_service_explained_right_image');
            if ($image) {
            echo '<img src="' . esc_url($image['url']) . '" alt="' . esc_attr($image['alt']) . '">';
            }
            ?>
    ...
    <div class="wrapper">
        <h3><?php the_field('ppc_industries_group_ppc_industries_heading')?></h3>
        <div class="main-seoinsight">
        <div class="semititle ftsz22"><?php the_field('ppc_industries_group_ppc_industries_subtitle')?></div>
        <div class="sec_absolutetxt"><?php the_field('ppc_industries_group_ppc_industries_content')?></div>
    </div>
        <?php
// Retrieve the group field
$service_group = get_field('ppc_industries_group'); // Replace 'service_group' with your group field name

if ($service_group && isset($service_group['ppc_industries_repeater'])): ?>
    <div class="servicebx_container fourcolumn flex-container wrap">
        <?php foreach ($service_group['ppc_industries_repeater'] as $service): 
            $service_link = $service['ppc_industries_url']; 
            $service_image = $service['ppc_industries_image']; 
            $service_name = $service['ppc_industries_text']; 
        ?>
